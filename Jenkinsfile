pipeline {
    ...
    environment {
        ...
        DOCKER_HUB_REPO = 'doneze/frontendservices'
    }
    ...
    stages {
        stage('Clone repositories') {
            steps {
                ...
                dir('frontend_repo') {
                    git url: 'https://github.com/Dezeabasili/microservices_frontend.git', 
                     branch: 'main', 
                     credentialsId: 'github-cred'
                }               
            }
        ...
        stage('Get Docker Image version') {
            steps {
                script {
                    def MAJOR_VERSION = readFile file: 'kubernetes_repo/tags_folder/frontend/major_version.txt'
                    def MINOR_VERSION = readFile file: 'kubernetes_repo/tags_folder/frontend/minor_version.txt'
                    def INITIAL_PATCH_VERSION = readFile file: 'kubernetes_repo/tags_folder/frontend/patch_version.txt'
                ...
                dir('kubernetes_repo') {
                    ...
                    sh '''
                        ...
                        sed -i "s|image:.*|image: $DOCKER_HUB_REPO:${DOCKER_TAG}|" \
                            kubernetes-manifests/microservices-folders/frontend/frontend-deployment.yaml

                        echo "$NEW_PATCH_VERSION" > tags_folder/frontend/patch_version.txt
                    '''
                }
                }
